...
export function LowLevelLab() {
  ...
  const assemblyRows = useMemo<readonly AssemblyRow[]>(() => {
    ...
    return nativeRows;
  }, [activeLanguage, hasRealArtifact, nativeRows, parsedProgram.instructions]);
  ...
  const panelContent: Record<PanelId, ReactNode> = {
    ...
    assembly: (
      <PanelFrame id="assembly" title="Assembly / Machine Code" meta={hasRealArtifact ? `${compiler} ${optimization} · objdump` : activeLanguage === "asm" ? `${assembler} · ${syntax}` : "awaiting compiler"} icon={<Binary size={12} />} maximized={maximizedPanel === "assembly"} onMaximize={() => setMaximizedPanel((current) => current === "assembly" ? null : "assembly")} onHide={() => togglePanel("assembly")} onDragStart={setDraggedPanel} onDrop={dropPanel} badge={<OriginChip real={hasRealArtifact}>{hasRealArtifact ? "REAL ARTIFACT" : activeLanguage === "asm" ? "SIM SOURCE" : "REFERENCE"}</OriginChip>}>
        <AssemblyPane rows={assemblyRows} hasRealArtifact={hasRealArtifact} activeSourceLine={highlightedSourceLine} currentSimulationSource={activeLanguage === "asm" ? currentEvent?.instruction.source : undefined} onSelectSourceLine={setAssemblySelectedLine} />
      </PanelFrame>
    ),
    visualizer: (
