...
import { AppWindow, ArrowRight, Binary, Boxes, Braces, Cpu, FlaskConical, Gamepad2, MemoryStick, Network, ScanLine, ShieldAlert } from "lucide-react";
...
const labs: readonly { title: string; eyebrow: string; description: string; href: string; icon: LucideIcon; tone: string; capabilities: readonly string[]; status: string }[] = [
  ...
  { title: "Security Lab", eyebrow: "VULNS / MALWARE RESEARCH", description: "Corrupção de memória, mitigações, fuzzing, amostras sintéticas, C2 isolado e Mini EDR — sempre em sandbox descartável.", href: "/labs/security", icon: ShieldAlert, tone: "rose", capabilities: ["Overflow visualizer", "Synthetic analysis", "Detection lab"], status: "INTERACTIVE" },
  { title: "Game Security Lab", eyebrow: "CHEAT RESEARCH / ANTI-CHEAT", description: "Jogo educacional, memória ao vivo, world-to-screen, Mini Anti-Cheat fictício e servidor loopback — nunca títulos online ou AC reais como alvo.", href: "/labs/game-security", icon: Gamepad2, tone: "amber", capabilities: ["Arena + offsets", "W2S / aim math", "Fictional AC"], status: "INTERACTIVE" }
] as const;
  ...
  return (
    <div className="labs-index-page">
      <header className="labs-index-header">
        ...
        <div className="labs-signal"><FlaskConical size={22} /><strong>09</strong><span>ACTIVE LABS</span><small>safe · local · educational</small></div>
      </header>
